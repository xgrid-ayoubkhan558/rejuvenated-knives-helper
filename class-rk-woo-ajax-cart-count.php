...
final class RK_Woo_Ajax_Cart_Count_Logic {
	...
	public static function shortcode( $atts = array() ) {
		...
		wp_enqueue_script(
			...
			plugin_dir_url( __FILE__ ) . 'assets/js/rk-woo-ajax-cart-count.js',
			array(),
			'1.0.0',
			true
		);
	...
	private static function render() {
		...
		ob_start();
		?>
		<a class="cart-custom-location rk-ajax-cart-count" href="<?php echo esc_url( $cart_url ); ?>" title="<?php echo esc_attr__( 'View Shopping Cart', 'rk-helper' ); ?>">
			<span style="padding: 10px" class="" aria-hidden="true"></span>
			<?php echo esc_html( sprintf( _n( '%d item ', '%d items ', $cart_count, 'rk-helper' ), $cart_count ) ); ?>&nbsp;<?php echo wp_kses_post( WC()->cart->get_cart_total() ); ?>
